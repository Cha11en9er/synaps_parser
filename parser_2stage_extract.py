"""
Шаг 2: копия листа из .env SHEET_UNIQUE → SHEET_FINAL, затем Synaps (parser_synaps_browser).

Лист «уникальные» не изменяется. Если листа «уникальные» или «финальные данные» нет — создаётся пустой.

Копирование на «финальные данные» — по совпадению ИНН (INN_PREFIX_COMPARE_ROWS) или PARSER_FORCE_COPY=1 в .env.
...
from parser_sheet_env import sheet_final, sheet_unique
